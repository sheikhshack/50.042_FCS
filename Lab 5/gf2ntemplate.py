...
class Polynomial2:
    ...
    def div(self, p2):
        ...
        while r.deg() >= d:
            s_content = (r.lc()/c)
            s_index = r.deg() - d
            s = Polynomial2(([0] * s_index).append(s_content))
            q = s.add(q)
            r = r.sub(s.mul(p2))
    ...
    def __str__(self):
        ...
        for i in range(coeff_len):
            if coefficients[i] == 1 and i != coeff_len-1:
                coeff_string += 'x^' + str(coeff_len - i - 1) + '+'
            elif coefficients[i] == 1 and i == coeff_len-1:
                coeff_string += 'x^' + str(coeff_len - i - 1)
...
############### My tests ###################3
# poly1 = Polynomial2([0,1,1,0,0,1])
# poly2 = Polynomial2([1,0,1,0,1,1])
# print(poly1.mul(poly2))

poly1 = Polynomial2([1,1,0,0,1,1,1,0,1])
poly2 = Polynomial2([1,0,1,0,1])
poly3 = Polynomial2([1,1,1,1,1])
print(poly1.add(poly2))

# poly1 = Polynomial2([1,1,0,0,1])
# poly2 = Polynomial2([1,0,1,1])
# poly1.div(poly2)
